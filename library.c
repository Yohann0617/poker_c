...
//    return result;
//}
unsigned char toByte(BitSequence *subEpsilon, int M, int nextBit, unsigned char base) {
    unsigned int a = 0;
...
extern int PokerDetect(int M, int n, BitSequence *epsilon) {
    int ret = 0;
    ...
    for (j = 0; j < N; ++j) {
        int startBit = j * M;
        int skipByte = startBit / 8;
        int nextBit = startBit % 8;

        flag = toByte(epsilon + skipByte, M, nextBit, base);
